...
export async function getPixAnalytics({
  role = "writer",
  // "full" builds the per-writer roster and reviewer table. The option remains
  // useful for lightweight callers, although both QA and admin use full scope.
  scope = "full",
  userLoginId = null,
  approverId = null,
  from = null,
  ...
  if (scope === "full" && (role === "qa" || role === "admin")) {
    const { rows: writerRows } = await getPool().query(
      `with filtered_posts as (
         select * from pix_posts ${where}
       ), writer_accounts as (
         select coalesce(nullif(u.display_name, ''), u.username, 'Unknown writer') as user_name,
                u.username,
                u.id::text as user_login_id,
                u.active,
                count(p.id)::int as sent_count,
                count(p.id) filter (where p.approved)::int as approved_count,
                count(p.id) filter (where p.rejected)::int as rejected_count,
                count(p.id) filter (where not p.approved and not p.rejected)::int as pending_count,
                round(coalesce((count(p.id) filter (where p.approved)) * 100.0 /
                  nullif(count(p.id), 0), 0))::int as approval_rate,
                count(p.id) filter (
                  where p.created_at >= ${todayStartSql} and p.created_at < ${tomorrowStartSql}
                )::int as today_count,
                count(p.id) filter (where p.created_at >= now() - interval '7 days')::int as week_count,
                count(p.id) filter (where p.created_at >= now() - interval '14 days'
                                      and p.created_at < now() - interval '7 days')::int as prev_week_count,
                max(p.created_at) as last_post_at
           from pix_users u
           left join filtered_posts p
             on p.user_login_id = u.id::text
             or lower(p.user_login_id) = lower(u.username)
          where u.role = 'writer'
          group by u.id, u.username, u.display_name, u.active
       ), historical_posts as (
         /* Posts with no matching writer account stay represented in one
            labelled row instead of splitting on every stored display name. */
         select 'Former / unlinked writers'::text as user_name,
                'Historical posts'::text as username,
                '__historical__'::text as user_login_id,
                false as active,
                count(p.id)::int as sent_count,
                count(p.id) filter (where p.approved)::int as approved_count,
                count(p.id) filter (where p.rejected)::int as rejected_count,
          ...
          where not exists (
            select 1 from pix_users u
             where u.role = 'writer'
               and (p.user_login_id = u.id::text or lower(p.user_login_id) = lower(u.username))
          )
         having count(p.id) > 0
       )
       select * from writer_accounts
       union all
       select * from historical_posts
    ...
    const { rows: qaRows } = await getPool().query(
      `with decisions as (
       ...
       ), reviewer_accounts as (
         select coalesce(nullif(u.display_name, ''), u.username, 'Unknown QA') as user_name,
                u.username,
                u.id::text as user_login_id,
                u.role,
                u.active,
                count(d.user_login_id) filter (where d.decision = 'approved')::int as approved_count,
                count(d.user_login_id) filter (where d.decision = 'rejected')::int as rejected_count,
                count(d.user_login_id) filter (
                  where d.decided_at >= ${todayStartSql} and d.decided_at < ${tomorrowStartSql}
                )::int as today_count,
                count(d.user_login_id) filter (where d.decided_at >= now() - interval '7 days')::int as week_count,
                count(d.user_login_id) filter (where d.decided_at >= now() - interval '14 days'
                                                and d.decided_at < now() - interval '7 days')::int as prev_week_count,
                max(d.decided_at) filter (where d.decision = 'approved') as latest_approval_at,
                max(d.decided_at) as latest_decision_at
           from pix_users u
           left join decisions d
             on d.user_login_id = u.id::text
             or lower(d.user_login_id) = lower(u.username)
          where u.role in ('qa', 'admin')
          group by u.id, u.username, u.display_name, u.role, u.active
       ), historical_decisions as (
         select 'Former / unlinked reviewers'::text as user_name,
                'Historical decisions'::text as username,
                '__historical__'::text as user_login_id,
                'qa'::text as role,
                false as active,
                count(d.user_login_id) filter (where d.decision = 'approved')::int as approved_count,
          ...
          where not exists (
            select 1 from pix_users u
             where u.role in ('qa', 'admin')
               and (d.user_login_id = u.id::text or lower(d.user_login_id) = lower(u.username))
          )
         having count(d.user_login_id) > 0
       )
       select * from reviewer_accounts
       union all
...
export async function getUserPixCounts(userId) {
  ...
  return rows[0] || { written_total: 0, written_week: 0, reviewed_total: 0, reviewed_week: 0 };
}
