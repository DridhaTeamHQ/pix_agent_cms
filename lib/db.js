...
export async function listPix({ limit = 50, offset = 0, userLoginId = null, viewerId = null, status = null, search = "" } = {}) {
  ...
  /* A list is ordered by the time of the thing it is a list OF.
     ...
     Each verdict tab therefore sorts by its own timestamp, and All by the most
     recent activity of any kind: GREATEST ignores nulls in Postgres and
     returns null only when every argument is null, so a row nothing has
     happened to falls back to created_at by itself, with no coalesce around
     each column.
  ...
  const orderBy = ORDER_BY[status]
    || "greatest(published_at, approved_at, rejected_at, updated_at, created_at) desc";
