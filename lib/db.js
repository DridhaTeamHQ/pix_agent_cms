...
export async function listPix({ limit = 50, offset = 0, userLoginId = null, status = null, search = "" } = {}) {
  ...
  /* A draft is not in the workflow yet. Excluded from every reviewer status
     — awaiting, approved, rejected — so QA's queue only ever holds work that
     was actually handed over. The author still sees their own drafts, which
     is what `drafts` is for. */
  if (status === "drafts") clauses.push("is_draft = true");
  else if (status !== "all-including-drafts") clauses.push("is_draft = false");
