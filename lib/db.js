...
export async function savePix(id, fields = {}, { ownerId = null, requireOpen = false } = {}) {
  ...
  if (id) {
    ...
    const { rows } = await getPool().query(
      `update pix_posts set ${sets.join(", ")} where id = $1${ownership}${settled} returning id, created_at, updated_at`,
      params
    );
 ...
 * records the change.
 */
export async function claimPublish(id, { byId = null } = {}) {
  await ensureSchema();
  const { rows } = await getPool().query(
    `update pix_posts
        set published_at = now(), published_by = $2
      where id = $1 and published_at is null and rejected = false
      returning id, published_at, published_by`,
    [id, byId]
  );
...
export async function recordPublishedId(id, publishedId) {
  ...
  const { rows } = await getPool().query(
    `update pix_posts set published_id = $2 where id = $1
      returning id, published_at, published_id`,
